...
public class Character2Controller2D : MonoBehaviour
{
    ...
    [Range(0, 1)][SerializeField] private float m_BackSpeed = .6f;
    [Range(0, .3f)][SerializeField] private float m_MovementSmoothing = .05f;   // How much to smooth out the movement
    [SerializeField] private bool m_AirControl = false;                         // Whether or not a player can steer while jumping;
    [SerializeField] private LayerMask m_WhatIsGround;                          // A mask determining what is ground to the character
    [SerializeField] private Transform m_GroundCheck;                           // A position marking where to check if the player is grounded.
    [SerializeField] private Collider2D m_CrouchDisableCollider;                // A collider that will be disabled when crouching
    [SerializeField] private Transform m_EnemyPlayer;
    ...
    {
        // Flip if crossing side
        if (m_GroundCheck.position.x + 1.1f < m_EnemyPlayer.position.x && isRight)
        {
            Flip();
            isRight = false;
        }
        else if (m_GroundCheck.position.x - 1.1f > m_EnemyPlayer.position.x && !isRight)
        {
            Flip();
            isRight = true;
        }
